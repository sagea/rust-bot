...
impl App {
    ...
    pub fn attempt_run(&mut self) {
        if self.status == AppStatus::Paused && self.is_point_in_bounds == true && self.menu_item_state == true {
            self.runner.sender.send(InEvents::Run).unwrap();
            self.set_status(AppStatus::Running);
        } else if self.status == AppStatus::Running && self.is_point_in_bounds == false {
            self.runner.sender.send(InEvents::Stop).unwrap();
            self.set_status(AppStatus::Pausing);
            self.set_run_count_current(0);
        } else if self.status == AppStatus::Paused && self.is_point_in_bounds == false {
            self.set_run_count_current(0);
        }
    }
}
